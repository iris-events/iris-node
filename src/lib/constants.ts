...
import * as connectionI from './connection.interfaces'
...
type ManagedExchangesReversedI = Record<ManagedExchangesE, keyof typeof ManagedExchangesE>

const managedExchangesReversed: ManagedExchangesReversedI = Object.keys(ManagedExchangesE).reduce((rev, managedExchangeKey): ManagedExchangesReversedI => {
  return {
    ...rev,
    [ManagedExchangesE[managedExchangeKey]]: managedExchangeKey,
  }
}, <ManagedExchangesReversedI>(<unknown>{}))

const NON_RESERVED_EXCHANGE_NAMES = [ManagedExchangesE.SUBSCRIPTION, ManagedExchangesE.SNAPSHOT_REQUESTED, ManagedExchangesE.SUBSCRIBE_INTERNAL]

export const CONNECTION_DEFAULT_OPTONS: Omit<connectionI.OptionalConfigI, 'socketOptions'> = {
  reconnectTries: 5,
...
const DEFAULTS: { EXCHANGE: ExchangeDefaultsI; QUEUE: QueueDefaultsI } = {
  EXCHANGE: {
    exchangeOptions: castToExchangeOptions({ durable: true, autoDelete: false }),
...
export const MANAGED_EXCHANGES = {
  FRONTEND: {
    ...
    EXCHANGE_OPTIONS: castToExchangeOptions({ durable: true, autoDelete: false }),
    QUEUE_OPTIONS: castToQueueOptions({ durable: true, autoDelete: false, messageTtl: DEFAULT_TTL }),
...
const RESERVED_EXCHANGE_NAMES = Object.values(ManagedExchangesE).filter(ex => !NON_RESERVED_EXCHANGE_NAMES.includes(ex))
...
export const getReservedNames = (): string[] => [...RESERVED_EXCHANGE_NAMES, ...RESERVED_QUEUE_NAMES]

export function getExchangeDefaultsForExchangeName(exchangeName: string): ExchangeDefaultsI {
  const managedExchange = <ManagedExchangeI | undefined>MANAGED_EXCHANGES[managedExchangesReversed[exchangeName]]

  return {
    exchangeOptions: managedExchange?.EXCHANGE_OPTIONS ?? DEFAULTS.EXCHANGE.exchangeOptions,
    exchangeType: managedExchange?.EXCHANGE_TYPE ?? DEFAULTS.EXCHANGE.exchangeType,
    scope: DEFAULTS.EXCHANGE.scope,
  }
}

export function getQueueDefaultsForExchangeName(exchangeName: string, overrideOptions: Partial<QueueDefaultsI>): QueueDefaultsI {
  const managedExchange = <{ QUEUE_OPTIONS?: QueueDefaultsI } | undefined>MANAGED_EXCHANGES[managedExchangesReversed[exchangeName]]
