...
export function validateProcessedHandlerConfig(
  ...
  if (
    processedHandlerConfig.messageDeliveryMode ===
    interfaces.MessageDeliveryMode.PER_SERVICE_INSTANCE
  ) {
    ...
    if (
      helper.classIsSameOrSubclassOf(
        <Function>handlerMetadata.messageClass,
        SnapshotRequested,
      )
    ) {
      throw new ValidationError(
...
export function validateBindingKeys(
  bindingKeys: string[],
  handlerMeta: interfaces.MessageHandlerMetadataI,
  msgMeta: message.ProcessedMessageMetadataI,
): void {
  const { exchangeType } = msgMeta.processedConfig
