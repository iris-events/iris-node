...
import { AsyncapiClassValidator, AsyncapiClassValidatorI } from '../class_validator'

const internallyDefinedMessages = <Function[]>[SubscribeInternal, ResourceMessage, SnapshotRequested]
...
export class AsyncapiSchema {
  ...
  constructor({ messages, messageHandlers, SCHEMA_POINTER_PREFIX, ...classValidatorOpts }: AsyncapiSchemaI) {
    this.messages = messages
    this.messageHandlers = messageHandlers
    this.SCHEMA_POINTER_PREFIX = SCHEMA_POINTER_PREFIX
    const asyncapiClassValidator = new AsyncapiClassValidator({ SCHEMA_POINTER_PREFIX, ...classValidatorOpts })
    this.irisSchemas = new IrisSchemas({ messages: messages, asyncapiClassValidator })
  ...
  public getChannels(): interfaces.AsyncChannelsObject {
    return this.messages
      .map(message => IrisChannels.generateChannels(message, this.messageHandlers, this.SCHEMA_POINTER_PREFIX))
      .flat()
      .reduce(
        (acc, channel) => ({
          ...acc,
  ...
  private additionallyDecorateMessages(): void {
    for (const message of this.messages) {
      ...
      this.decorateMessageWithIrisAdditionalProperties(<Function>message.target)
    }
  }

  private assureMessageIsIncludedInSchema<T extends Function>(message: T): void {
  ...
  private isOurMessage<T extends Function>(message: T): boolean {
    ...
    return this.messageHandlers.find(handler => handler.messageClass === message) === undefined
  }
