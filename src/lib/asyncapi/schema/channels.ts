...
abstract class Channel {
  ...
  protected message2Spec(): interfaces.AsyncMessageObject | interfaces.ReferenceObject {
  ...
  protected message2AmqpBindings(): interfaces.AmqpChannelBindingExchange | interfaces.AmqpChannelBindingQueue {
    const { exchangeOptions, exchangeName, exchangeType } = this.message.processedConfig
  ...
  protected additionalMessageProperties(intermediate: interfaces.AsyncMessageObject): interfaces.AsyncMessageObject {
    return {
  ...
  protected getAdditionalMessageSchemaProperties(): Record<string, interfaces.SchemaObject | interfaces.ReferenceObject> {
    return {}
  }
}
...
export class PublishChannel extends Channel {
  ...
  constructor(schemaPointerPrefix: string, message: ProcessedMessageMetadataI, handler: ProcessedMessageHandlerMetadataI) {
...
export class IrisChannels {
  private static subscriptionChannelClass: typeof SubscriptionChannel = SubscriptionChannel
  ...
  public static generateChannels(
    ...
    const handlers = messageHandlers.filter(mh => mh.messageClass === message.target)
    if (handlers.length === 0) {
      return new this.subscriptionChannelClass(schemaPointerPrefix, message)
    }

    return handlers.map(handler => new this.publishChannelClass(schemaPointerPrefix, message, handler))
  }

  public static setCustomChannelClasses({ publishChannelClass, subscriptionChannelClass }: CustomChannelClassesI): IrisChannels {
    if (publishChannelClass !== undefined) {
      this.publishChannelClass = publishChannelClass
    }
    if (subscriptionChannelClass !== undefined) {
      this.subscriptionChannelClass = subscriptionChannelClass
    }
