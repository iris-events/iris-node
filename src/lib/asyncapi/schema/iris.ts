...
import type { ProcessedMessageMetadataI } from '../../message.interfaces'
import type { AsyncapiClassValidator } from '../class_validator'
...
export class IrisSchemas {
  ...
  private pickRelevantSchemas(): interfaces.SchemaObjects {
    ...
    function searchObjForRefs(
      obj?: interfaces.SchemaObject | unknown[] | null,
    ): void {
      if (obj !== null && (Array.isArray(obj) || typeof obj === 'object')) {
        for (const prop of Object.values(obj).flat()) {
          if (Array.isArray(prop)) {
            searchObjForRefs(prop)
          } else if (typeof prop === 'object') {
            if ((<{ $ref?: string }>prop).$ref !== undefined) {
              const sname = <string>(
                (<string>(<{ $ref?: string }>prop).$ref).split('/').pop()
              )
      ...
      .map(
        (sname) =>
          (<{ properties?: Record<string, unknown> }>(
            schemas[<string>eventSchemas[sname]]
          )).properties,
      )
  ...
  private getBaseEventSchemasFromMessages(
    schemas: interfaces.SchemaObjects,
  ): Record<string, string | undefined> {
    return this.messages.reduce((acc, msgMeta) => {
      const { targetClassName: cName } = msgMeta
      const hasSchema =
        <interfaces.SchemaObject | undefined>schemas[cName] !== undefined

      if (!hasSchema) {
        throw new Error(
          `ERR_IRIS_ASYNCAPI_MESSAGE_SCHEMA_NOT_FOUND_FOR: ${cName}`,
        )
      }
  ...
  private fixDateTimeProperties(
    schemas: interfaces.SchemaObjects,
  ): interfaces.SchemaObjects {
    // Date becomes
      ...
      .each(
        (
          ...
          _(props)
            .values()
            .each(
              (prop: interfaces.SchemaObject | interfaces.ReferenceObject) => {
                ...
                const datesOnly =
                  oneOf
                    .map((oo) => oo.format)
                    .filter((fmt) => fmt === 'date' || fmt === 'date-time')
                    .length === 2
