 ...
 * Only use for tests.
 */
import { MANAGED_EXCHANGES, ManagedExchangesE, ManagedExchangeI } from './constants'
...
type ReservedManagedExchangesE = Exclude<
  ManagedExchangesE,
  ManagedExchangesE.SUBSCRIPTION | ManagedExchangesE.SNAPSHOT_REQUESTED | ManagedExchangesE.SUBSCRIBE_INTERNAL | ManagedExchangesE.FRONTEND
>

const reservedExchangesToRegister: Record<ReservedManagedExchangesE, ManagedExchangeI> = {
...
export async function registerFrontendExchange(): Promise<void> {
  const { EXCHANGE, EXCHANGE_TYPE, EXCHANGE_OPTIONS } = MANAGED_EXCHANGES.FRONTEND
...
export async function registerReservedManagedMessages(): Promise<void> {
  await Promise.all(
    Object.values(reservedExchangesToRegister).map(async ({ EXCHANGE, EXCHANGE_TYPE, EXCHANGE_OPTIONS }) =>
...
export function getReservedManagedExchangeNames(): string[] {
  return Object.values(reservedExchangesToRegister).map(({ EXCHANGE }) => EXCHANGE)
}
