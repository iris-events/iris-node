...
export type queueOptionsT = amqplib.Options.AssertQueue & {
	durable: boolean;
	autoDelete: boolean;
	exclusive: boolean;
};
...
export interface MessageHandlerMetadataNoReplyI
	extends MessageHandlerMetadataBaseI {
	kind: "NO_REPLY";
}
export interface MessageHandlerMetadataWithReplyI
	extends MessageHandlerMetadataBaseI {
	kind: "WITH_REPLY";
...
export type MessageHandlerMetadataI =
	| MessageHandlerMetadataWithReplyI
	| MessageHandlerMetadataNoReplyI;

export type ProcessedMessageHandlerMetadataI = MessageHandlerMetadataI & {
	processedConfig: ProcessedMessageHandlerConfigI;
};

export interface ProcessedMessageHandlerConfigI
	extends Omit<MessageHandlerI, "bindingKeys" | "autoDelete" | "durable"> {
	bindingKeys: string[];
	messageDeliveryMode: MessageDeliveryMode;
