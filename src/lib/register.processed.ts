...
async function assertExchangeAndQueues(
  ...
  const handlers = messageHandlers.filter(mh => mh.messageClass === msgMeta.target)
  for (const handler of handlers) {
    const obtainChannel = async (): Promise<amqplib.Channel> => connection.assureChannelForHandler(handler)
...
async function assertExchange(msg: message.ProcessedMessageMetadataI): Promise<void> {
  if (message.isFrontend(msg)) {
    await initFrontendQueue()
  } else {
    await amqpHelper.assertExchange(msg.processedConfig.exchangeName, msg.processedConfig.exchangeType, msg.processedConfig.exchangeOptions)
  }
}

async function registerMessageHandler(handler: messageHandler.ProcessedMessageHandlerMetadataI, msgMeta: message.ProcessedMessageMetadataI): Promise<void> {
  const channel = await connection.assureChannelForHandler(handler)
  const { exchange, queueName, queueOptions, bindingKeys } = handler.processedConfig
...
async function registerDeadletter(handler: messageHandler.ProcessedMessageHandlerMetadataI, msgMeta: message.ProcessedMessageMetadataI): Promise<void> {
  const { deadLetterIsCustom } = msgMeta.processedConfig
  const { deadLetterExchange, deadLetterRoutingKey } = handler.processedConfig.queueOptions

  if (!deadLetterIsCustom || deadLetterExchange === undefined || deadLetterRoutingKey === undefined) {
    return
  }

  logger.debug(`Register custom DQL for '${handler.processedConfig.queueName}'`, {
    ...handler.processedConfig.queueOptions,
  })

  await amqpHelper.assertExchange(deadLetterExchange, DEAD_LETTER.EXCHANGE_TYPE, DEAD_LETTER.EXCHANGE_OPTIONS)
...
async function initFrontendQueue(): Promise<void> {
  const fronendQueueName = amqpHelper.getFrontendQueueName()
  await Promise.all([featManagement.registerFrontendExchange(), amqpHelper.assertQueue(fronendQueueName, FRONTEND.QUEUE_OPTIONS)])
}
