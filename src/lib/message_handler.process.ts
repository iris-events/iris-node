...
import { getQueueDefaultsForExchangeName, MANAGED_EXCHANGES } from './constants'
...
export function processAndValidateConfig(
  ...
  const messageDeliveryMode = origConfig.messageDeliveryMode ?? interfaces.MessageDeliveryMode.PER_SERVICE
  let { durable, autoDelete } = getQueueDefaultsForExchangeName(exchangeName, origConfig)

  if (messageDeliveryMode === interfaces.MessageDeliveryMode.PER_SERVICE_INSTANCE) {
    durable = false
    autoDelete = true
  }
  ...
  const processedConfig: Omit<interfaces.ProcessedMessageHandlerConfigI, 'queueName' | 'queueOptions'> = {
    ..._.omit(origConfig, ['durable', 'autoDelete']),
    messageDeliveryMode,
    exchange: message.isFrontend(msgMeta) ? FRONTEND.EXCHANGE : exchangeName,
    bindingKeys: getBindingKeys(configIntermediate, origConfig.bindingKeys, msgMeta),
  }

  const queueName = getQueueName(configIntermediate.uuid, processedConfig, msgMeta)
  ...
  validation.validateProcessedHandlerConfig(processedHandlerConfig, configIntermediate, msgMeta)
...
function getQueueName(
  handlerUuid: string,
  config: Pick<interfaces.ProcessedMessageHandlerConfigI, 'messageDeliveryMode' | 'bindingKeys'>,
  msgMeta: message.ProcessedMessageMetadataI
): string {
  ...
  const appendBindingKeys = (exchangeType === message.ExchangeType.direct || exchangeType === message.ExchangeType.topic) && !message.isFrontend(msgMeta)
...
function getQueueExtraOptions(queueName: string, msgMeta: message.ProcessedMessageMetadataI): Partial<amqplib.Options.AssertQueue> {
