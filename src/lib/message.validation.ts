...
export function validateProcessedMessageConfig(
  msgConfig: interfaces.MessageI,
  target: Object,
): void {
  ...
  if (_.isEmpty(name)) {
    throw new ValidationError(
      'MESSAGE_REQUIRES_NAME',
      `@Message(${targetClassName}) requires a non empty name to be set`,
      {
  ...
  validationIris.throwIfValueIsInvalidCaseFormat(
    ...
    `Name for @Message(${helper.getTargetConstructor(target).name})`,
  )
  ...
  if (deadLetter !== undefined) {
    validationIris.throwIfValueIsInvalidCaseFormat(
      deadLetter,
      'MESSAGE_INVALID_DEAD_LETTER_NAME',
      `DeadLetter for @Message(${targetClassName})`,
    )
  }
}
