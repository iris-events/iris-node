...
export function getErrorMessage(error: Error): ErrorMessageI {
	const message =
		error instanceof MsgError ? error.getMessage() : error.message;
...
export function isRejectableError(error: Error): boolean {
	...
	if (error instanceof MsgError) {
		...
		return (
			errorType === ErrorTypeE.AUTHORIZATION_FAILED ||
			errorType === ErrorTypeE.UNAUTHORIZED ||
			errorType === ErrorTypeE.FORBIDDEN
		);
	}
